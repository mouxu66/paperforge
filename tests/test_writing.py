...
def test_create_chapter_with_parent(db: Session):
    ...
    crud.create_chapter(db, proj.id, ChapterCreate(title="研究背景", parentId=intro.id))
    tree = crud.get_chapter_tree(db, proj.id)
...
def test_create_user_template(db: Session):
    ...
    payload = UserTemplateCreate(
        ...
        description="用于测试",
        chapters=[
            TemplateChapter(title="引言", children=[]),
...
def test_delete_user_template(db: Session):
    """删除用户模板。"""
    tmpl = crud.create_user_template(db, UserTemplateCreate(name="待删除", chapters=[]))
...
def test_export_replaces_citation_markers(db: Session):
    ...
    intro = crud.get_chapter_tree(db, proj.id)[0]
    crud.update_chapter(
        db, intro.id, ChapterUpdate(content="本文基于 Transformer 架构[@1706.03762]展开讨论。")
    )
...
def test_export_preserves_citation_order(db: Session):
    ...
    intro = crud.get_chapter_tree(db, proj.id)[0]
    crud.update_chapter(
        db, intro.id, ChapterUpdate(content="先引[@paper_b]再引[@paper_a]再引[@paper_b]")
    )
...
def test_export_no_citations(db: Session):
    ...
    crud.update_chapter(
        db,
        crud.get_chapter_tree(db, proj.id)[0].id,
...
# CSL-JSON 导出
# ===========================================================================
def test_export_project_csl_journal_article(db: Session):
    """期刊论文导出为 CSL-JSON 时类型为 article-journal 并含 container-title。"""
    from mock_api.models import Paper
    ...
    proj = crud.create_project(db, ProjectCreate(title="CSL Test"))
    intro = crud.get_chapter_tree(db, proj.id)[0]
    crud.update_chapter(db, intro.id, ChapterUpdate(content="引用[@journal-001]"))

    items = crud.export_project_csl(db, proj.id)
    assert items is not None
    assert len(items) == 1
    item = items[0]
    assert item["id"] == "journal-001"
    assert item["type"] == "article-journal"
    assert item["title"] == "Deep Learning for Science"
    assert item["container-title"] == "Nature"
    assert item["issued"] == {"date-parts": [[2025]]}
    assert len(item["author"]) == 2
    assert item["author"][0] == {"family": "LeCun", "given": "Yann"}
    assert item["author"][1] == {"family": "Bengio", "given": "Yoshua"}


def test_export_project_csl_arxiv_preprint(db: Session):
    """arXiv ID 导出为 CSL-JSON 时类型为 article 并生成 arXiv URL。"""
    from mock_api.models import Paper
    ...
    db.add(
        Paper(
            id="1706.03762",
            title="Attention Is All You Need",
            authors=["Ashish Vaswani", "Shazeer, Noam"],
            year=2017,
            abstract="",
            category="cs",
        )
    ...
    proj = crud.create_project(db, ProjectCreate(title="ArXiv CSL"))
    intro = crud.get_chapter_tree(db, proj.id)[0]
    crud.update_chapter(db, intro.id, ChapterUpdate(content="引用[@1706.03762]"))

    items = crud.export_project_csl(db, proj.id)
    assert items is not None
    assert len(items) == 1
    item = items[0]
    assert item["id"] == "1706.03762"
    assert item["type"] == "article"
    assert item["URL"] == "https://arxiv.org/abs/1706.03762"
    assert item["author"][0] == {"family": "Vaswani", "given": "Ashish"}
    assert item["author"][1] == {"family": "Shazeer", "given": "Noam"}


def test_export_project_csl_doi_id(db: Session):
    """DOI 形式的引用 ID 应被识别并写入 DOI 字段。"""
    from mock_api.models import Paper
    ...
    proj = crud.create_project(db, ProjectCreate(title="DOI CSL"))
    intro = crud.get_chapter_tree(db, proj.id)[0]
    crud.update_chapter(db, intro.id, ChapterUpdate(content=f"引用[@{doi_id}]"))

    items = crud.export_project_csl(db, proj.id)
    assert items is not None
    assert len(items) == 1
    item = items[0]
    assert item["id"] == doi_id
    assert item["type"] == "article-journal"
    assert item["DOI"] == doi_id


def test_export_project_csl_missing_paper(db: Session):
    """引用不存在的论文 → 仍生成占位 CSL 条目，类型为 document。"""
    proj = crud.create_project(db, ProjectCreate(title="Missing CSL"))
    intro = crud.get_chapter_tree(db, proj.id)[0]
    crud.update_chapter(db, intro.id, ChapterUpdate(content="引用[@ghost_paper]"))

    items = crud.export_project_csl(db, proj.id)
    assert items is not None
    assert len(items) == 1
    item = items[0]
    assert item["id"] == "ghost_paper"
    assert item["type"] == "document"
    assert "未找到" in item["title"]


def test_export_project_csl_nonexistent_project(db: Session):
    """项目不存在 → None。"""
    assert crud.export_project_csl(db, 9999) is None
...
# 续写 / 改写 provenance
# ===========================================================================
def test_create_continuation_stamps_kind(db: Session):
    """create_continuation 按传入 kind 落库，供导出生成 AI 使用声明。"""
    proj = crud.create_project(db, ProjectCreate(title="P"))
    intro = crud.get_chapter_tree(db, proj.id)[0]

    rec_continue = crud.create_continuation(db, intro.id, "续写内容", "继续", kind="continue")
    rec_rewrite = crud.create_continuation(db, intro.id, "改写内容", "", kind="rewrite")

    assert rec_continue["kind"] == "continue"
    assert rec_rewrite["kind"] == "rewrite"


def test_list_continuations_includes_kind(db: Session):
    """list_continuations 返回的记录应包含 kind。"""
    proj = crud.create_project(db, ProjectCreate(title="P"))
    intro = crud.get_chapter_tree(db, proj.id)[0]
    crud.create_continuation(db, intro.id, "A", "", kind="continue")
    crud.create_continuation(db, intro.id, "B", "", kind="rewrite")

    rows = crud.list_continuations(db, intro.id)
    assert len(rows) == 2
    kinds = {r["kind"] for r in rows}
    assert kinds == {"continue", "rewrite"}


def test_get_project_ai_usage_counts_kinds(db: Session):
    """get_project_ai_usage 分别统计续写、改写及涉及章节数。"""
    proj = crud.create_project(db, ProjectCreate(title="P"))
    intro = crud.get_chapter_tree(db, proj.id)[0]
    crud.create_chapter(db, proj.id, ChapterCreate(title="方法"))
    method = crud.get_chapter_tree(db, proj.id)[1]

    crud.create_continuation(db, intro.id, "续写1", "", kind="continue")
    crud.create_continuation(db, intro.id, "续写2", "", kind="continue")
    crud.create_continuation(db, method.id, "改写", "", kind="rewrite")

    usage = crud.get_project_ai_usage(db, proj.id)
    assert usage["has_ai"] is True
    assert usage["continuation_count"] == 2
    assert usage["rewrite_count"] == 1
    assert usage["chapters_with_ai"] == 2
...
def test_note_crud(db: Session):
    ...
    db.add(
        Paper(
            id="note_test", title="笔记测试论文", authors=[], year=2020, abstract="", category="x"
        )
    )
    db.commit()
    ...
    n1 = crud.create_note(
        db, NoteCreate(paperId="note_test", projectId=proj.id, content="第一条笔记")
    )
