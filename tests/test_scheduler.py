...
@patch("mock_api.database.SessionLocal")
def test_run_fetch_once_search_failure_continues_other_keywords(
    mock_session, mock_search
):
...
# start_scheduler
# ===========================================================================
def test_start_scheduler_no_keywords_does_not_start():
    """未配置关键词 → 不启动调度器（_scheduler 保持 None）。"""
    with patch.object(scheduler, "ARXIV_AUTO_FETCH_KEYWORDS", []):
        scheduler.start_scheduler()
    assert scheduler._scheduler is None


def test_start_scheduler_idempotent_when_already_running():
    """已启动 → 直接返回，不重复创建。"""
    fake_scheduler = MagicMock()
    scheduler._scheduler = fake_scheduler

    with patch.object(scheduler, "ARXIV_AUTO_FETCH_KEYWORDS", ["LLM"]):
        scheduler.start_scheduler()
...
def test_start_scheduler_apscheduler_missing_degrades_gracefully():
...
def test_start_scheduler_with_keywords_creates_scheduler():
    ...
    with patch.object(scheduler, "ARXIV_AUTO_FETCH_KEYWORDS", ["LLM"]):
        with patch(
            "apscheduler.schedulers.background.BackgroundScheduler", return_value=fake_bg
        ):
            scheduler.start_scheduler()

    assert scheduler._scheduler is fake_bg
    fake_bg.add_job.assert_called_once()
