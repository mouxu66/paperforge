...
from mock_api.llm.factory import LLMFactory, detect_provider, _mask_api_key
...
def test_empty_configs_raises_runtime_error():
    ...
    factory = LLMFactory()
    with patch.object(factory, "_load_configs_from_db", return_value=[]):
        with pytest.raises(RuntimeError, match="未配置任何大模型"):
...
def test_switch_model_changes_provider():
    ...
    with patch.object(factory, "_load_configs_from_db", return_value=[config1, config2]), \
         patch.object(factory, "_load_config_by_id", side_effect=lambda cid: {1: config1, 2: config2}.get(cid)):
        # 首次调用：自动选择第一个启用的配置
...
def test_switch_to_disabled_model_raises():
    ...
    with patch.object(factory, "_load_configs_from_db", return_value=[config]), \
...
def test_available_models():
    ...
    configs = [
        ...
        _make_config(config_id=2, display_name="GLM", api_url="https://open.bigmodel.cn/api/paas/v4"),
    ]
