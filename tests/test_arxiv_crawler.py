...
def test_search_arxiv_success():
    ...
    # arXiv cs.* 子类应映射到大学科门类「工学」
    assert first["category"] == "engineering"
...
def test_search_arxiv_network_error():
    ...
    with (
        patch(
            "mock_api.arxiv_crawler.requests.get", side_effect=requests.RequestException("timeout")
        ),
        pytest.raises(RuntimeError, match="请求 arXiv API 失败"),
    ):
...
def test_search_arxiv_non_200():
    ...
    with (
        patch("mock_api.arxiv_crawler.requests.get", return_value=mock_resp),
        pytest.raises(RuntimeError, match="非 200"),
    ):
        search_arxiv("neural network")
