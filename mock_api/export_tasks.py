...
服务两类导出：
- 写作项目导出：POST /api/writing/projects/{id}/export → start_export_task
  返回 task_id；GET /api/writing/export/{task_id}/progress 轮询进度。
  P2-2 增强：章节分批渲染（current_chapter / total_chapters）+ 5 分钟缓存。
- 全班诚信报告批量打包：POST /api/reports/integrity/export-batch →
  start_integrity_batch_task（zip 落盘路径写入 result_path）。
...
class ExportTask:
    ...
    - project_id：被导出的写作项目 ID（诚信报告批量任务不适用，占位 0）
    - result_path：status=done 时批量 zip 的落盘路径（写作导出不用此字段）
    ...
    # 2026-08-05: 诚信报告批量打包 —— zip 文件落盘路径（status=done 时有效）
    result_path: str = ""
...
def get_export_task(task_id: str) -> ExportTask | None:
    """查询任务进度。任务不存在（或已被淘汰）时返回 None。"""
    with _lock:
        return _tasks.get(task_id)


def start_integrity_batch_task() -> str:
    """启动「全班诚信报告批量打包（zip）」异步任务，返回 task_id。

    复用本模块的任务表 / 线程调度 / FIFO 淘汰：
    - 后台 daemon 线程执行 _run_integrity_batch
    - 进度写入 ExportTask.progress，zip 落盘路径写入 result_path
    """
    task_id = uuid.uuid4().hex
    task = ExportTask(task_id=task_id, project_id=0)  # project_id 不适用，占位 0
    with _lock:
        _cleanup_old_tasks_locked()
        _tasks[task_id] = task
    t = threading.Thread(
        target=_run_integrity_batch,
        args=(task_id,),
        daemon=True,
        name=f"integrity-batch-{task_id[:8]}",
    )
    t.start()
    return task_id


def _run_integrity_batch(task_id: str) -> None:
    """后台线程：遍历全部感悟报告，打包 zip 并记录落盘路径。"""
    from .integrity_report import render_integrity_batch_zip
    ...
    try:
        ...
        def progress_cb(current: int, total: int) -> None:
            # 5% → 90% 区间映射渲染进度
            _set_progress(task_id, 5 + int(90 * current / max(total, 1)))

        result_path = render_integrity_batch_zip(db, progress_cb=progress_cb)
        _set_batch_done(task_id, str(result_path))
    except Exception as e:  # noqa: BLE001 - 后台线程必须吞下所有异常
        _set_error(task_id, f"批量导出失败：{e}")
    finally:
        db.close()
...
def _run_export(task_id: str, project_id: int) -> None:
    ...
    try:
        ...
        result = crud.export_project_markdown(db, project_id, progress_cb=progress_cb)
...
def _set_batch_done(task_id: str, result_path: str) -> None:
    """标记批量打包任务完成并写入 zip 路径（线程安全）。"""
    with _lock:
...
def _cleanup_old_tasks_locked() -> None:
    ...
    if len(_tasks) <= MAX_TASKS:
        return
    # 只淘汰终态任务；运行中的任务必须保留，后台线程完成后仍需
    # 能写回 result_path，不能因新任务洪峰而让客户端拿到“任务不存在”。
    terminal_ids = sorted(
        (tid for tid, task in _tasks.items() if task.status in ("done", "error")),
