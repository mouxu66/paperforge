...
from mock_api.models import PaperEmbedding as PaperEmbeddingORM
...
def embed_all_papers(force: bool = False) -> None:
    ...
    try:
        ...
        # 4. 过滤已有向量（除非 force）
        if not force:
            existing_ids = {r[0] for r in db.query(PaperEmbeddingORM.paper_id).all()}
        ...
        texts = [f"{p.title}. {p.abstract or ''}".strip() for p in pending]
