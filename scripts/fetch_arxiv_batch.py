...
def fetch_with_retry(keyword: str, max_results: int) -> tuple[list[dict], Optional[str]]:
    """带指数退避重试的 arXiv 拉取。
...
def import_papers(base_url: str, papers: list[dict]) -> tuple[int, int, list[str]]:
    ...
    try:
        ...
        failed_ids = [
            r["id"] for r in data.get("results", []) if not r.get("success", True)
        ]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="按关键词批量拉取 arXiv 论文并入库"
    )
    parser.add_argument(
        "--base-url",
    ...
    # 健康检查
    print("=" * 60)
    print(f"PaperForge arXiv 批量拉取工具")
    ...
    try:
        ...
        print(f"[错误] 后端不可达：{e}")
        print(f"       请先启动后端：python -m mock_api.main")
        sys.exit(1)
