...
背景（2026-08-12）：旧版 R1.5 把「有效证据 2-4 条」的 4 维分数一律封顶 0.85，
41 篇全量里 UA=0.85 的有 36 篇、ES=0.85 有 35 篇 —— 硬帽本身在压平分数。
本脚本用冒烟 4 篇验证：把统一帽换成分级帽（证据 2 条→0.75、3 条→0.80、4 条→0.85）
能否在保留校验语义的同时拉开区分度。

【已落地】分级帽（GRADED_CAP_BY_EVIDENCE）已进入 depth_eval_reflection.py 生产路径，
本脚本保留作历史 A/B 证据；"A 现行0.85帽" 现指旧版统一帽，新 A/B 应改对比生产分级帽。
