...
# 带重试的请求执行
# ---------------------------------------------------------------------------
def execute_request_with_retry(base_url: str, case: dict, case_id: str, completed_path: Path) -> dict:
    """执行单个测试用例，支持指数退避重试。"""
...
def generate_report(all_results: dict[str, list[dict]], output_dir: Path) -> None:
    ...
    all_times = [r["response_time_ms"] for r_list in all_results.values() for r in r_list if r["response_time_ms"] is not None]
    ...
    for endpoint_key, results in all_results.items():
        ...
        lines.append(f"### 2.{list(all_results.keys()).index(endpoint_key) + 1} {name}（`{endpoint_key}`）")
        lines.append("")
        ...
        lines.append(f"**按主题分布：**")
        lines.append("")
        lines.append(f"| 主题 | 用例数 | 成功率 | 平均响应时间 | 平均输出 Token |")
        lines.append(f"|------|--------|--------|-------------|---------------|")
        for t, s in topic_stats.items():
            ...
            lines.append(f"| {t} | {s['count']} | {s['success'] / s['count'] * 100:.0f}% | {avg_time} | {avg_tok} |")
        lines.append("")

        # 失败详情
        if failed:
            lines.append(f"**失败用例（前 5 条）：**")
            lines.append("")
            for r in failed[:5]:
                lines.append(f"- `{r['payload_summary'][:80]}` → {r.get('error', '未知错误')[:100]}")
            lines.append("")
    ...
    lines.append(f"| 主题 | 总用例 | 成功 | 成功率 | 平均响应时间 | 总 Token |")
    lines.append(f"|------|--------|------|--------|-------------|---------|")
    for t, s in all_topic_stats.items():
        avg_time = f"{sum(s['times']) / len(s['times']):.0f} ms" if s["times"] else "N/A"
        lines.append(f"| {t} | {s['count']} | {s['success']} | {s['success'] / s['count'] * 100:.0f}% | {avg_time} | {sum(s['tokens']):,} |")
    lines.append("")
    ...
    if all_times:
        ...
        if p95 > 10000:
            ...
            lines.append(f"- P95 响应时间 {p95:.0f}ms，属于可接受范围，流式接口可进一步优化用户体验。")
        else:
            lines.append(f"- P95 响应时间 {p95:.0f}ms，性能良好。")
    lines.append(f"- 估算总 Token 消耗：{total_tokens:,} tokens（含输入 + 输出）。")
    lines.append(f"- 实际 Token 消耗约为估算值的 2-3 倍（输入 Prompt 通常比输出更长）。")
    lines.append("")
...
def main() -> None:
    ...
    if completed_cases:
        skipped = sum(1 for i, case in enumerate(cases_to_run, start=start_index)
    ...
    for i, case in enumerate(cases_to_run, start=start_index):
        ...
        if completed_count % 10 == 0 or completed_count == len(cases_to_run):
            ...
            print(f"  [{completed_count}/{len(cases_to_run)}] {status} {case['topic'][:6]} | {time_str} | {result['output_tokens_est']} tok | {result['payload_summary'][:50]}")
            print(f"    ↳ 进度：{completed_count}/{len(cases_to_run)} | 耗时：{elapsed:.0f}秒 | 连续失败：{consecutive_failures}")
