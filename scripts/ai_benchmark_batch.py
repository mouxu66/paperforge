...
def generate_outline_batches(project_id: int, batch_size: int = 5, total: int = 20) -> list[dict]:
    ...
    for i in range(0, len(all_topics), batch_size):
        ...
        numbered = "\n".join(
            f"[{j+1}] {item['outline_topic']}" for j, item in enumerate(chunk)
        )
        composite_topic = (
...
def generate_continue_batches(chapter_id: int = 1, batch_size: int = 5, total: int = 20) -> list[dict]:
    """生成续写批处理用例。"""
    all_directions = []
    for topic_data in TOPICS.values():
    ...
    for i in range(0, len(all_directions), batch_size):
        ...
        numbered = "\n".join(
            f"[{j+1}] {item['direction']}" for j, item in enumerate(chunk)
        )
        n = len(chunk)
        composite_direction = (
...
def generate_rewrite_batches(chapter_id: int = 1, batch_size: int = 5, total: int = 20) -> list[dict]:
    """生成改写批处理用例。"""
    all_texts = []
    for topic_data in TOPICS.values():
    ...
    for i in range(0, len(all_texts), batch_size):
        ...
        numbered = "\n\n".join(
            f"[原文{j+1}] {item['text']}" for j, item in enumerate(chunk)
        )
        n = len(chunk)
        composite_text = (
...
def generate_suggest_batches(chapter_id: int = 1, batch_size: int = 5, total: int = 15) -> list[dict]:
...
def generate_search_batches(batch_size: int = 5, total: int = 15) -> list[dict]:
    """生成语义搜索批处理用例。"""
    all_queries = []
    for topic_data in TOPICS.values():
        for q in topic_data["questions"]:
    ...
    for i in range(0, len(all_queries), batch_size):
        ...
        numbered = "\n".join(
            f"[{j+1}] {item['query']}" for j, item in enumerate(chunk)
        )
        composite_query = (
            f"请搜索与以下 {len(chunk)} 个查询相关的论文：\n{numbered}"
        )
        batches.append({
            "endpoint": "/api/search/semantic",
            "payload": {"question": composite_query},
            "sub_cases": chunk,
            "sub_cases_count": len(chunk),
            "batch_label": f"search_batch_{i // batch_size + 1}",
        })
    return batches
...
def execute_batch_with_retry(base_url: str, batch: dict, batch_id: str, completed_path: Path) -> dict:
    """执行一个批处理请求（带指数退避重试）。"""
    ...
    for attempt in range(MAX_RETRIES + 1):
        ...
        if attempt < MAX_RETRIES:
            ...
            result["sub_cases_count"] = len(sub_cases)
            result["sub_cases"] = [
                {"topic": c["topic"], "success": False, "error": result.get("error")}
                for c in sub_cases
            ]
            return result
...
def execute_sequential_batch(base_url: str, batch: dict, batch_id: str, completed_path: Path) -> dict:
...
def save_completed(path: Path, batch_id: str) -> None:
    ...
    path.write_text(json.dumps({"completed": list(completed)}, ensure_ascii=False, indent=2), "utf-8")
...
def generate_report(all_results: list[dict], output_dir: Path) -> None:
    ...
    total_success = sum(
        sum(1 for s in r.get("sub_cases", []) if s.get("success"))
    ...
    lines = [
        ...
        f"| 失败用例数 | {total_fail} |",
        f"| 用例成功率 | {total_success / total_sub * 100:.1f}% |" if total_sub > 0 else "| 用例成功率 | N/A |",
    ]
    if all_times:
        lines += [
            f"| 平均请求耗时 | {sum(all_times) / len(all_times):.0f} ms |",
            f"| 最快请求 | {min(all_times):.0f} ms |",
            f"| 最慢请求 | {max(all_times):.0f} ms |",
        ]
    lines += [
    ...
    for idx, (func_key, results) in enumerate(func_groups.items(), 1):
        ...
        lines += [
            ...
            f"| 平均请求耗时 | {avg_time:.0f} ms |",
            f"| 总输出 Token | {total_tok:,} |",
            "",
        ]

    # 各批次明细
    lines += ["## 3. 批次明细", "", "| 批次 ID | 请求数 | 用例数 | 成功 | 耗时(ms) | Token |", "|---------|--------|--------|------|---------|-------|"]
    for r in all_results:
        sub_count = r.get("sub_cases_count", 0)
        sub_ok = sum(1 for s in r.get("sub_cases", []) if s.get("success"))
        lines.append(
            f"| {r['batch_id']} | 1 | {sub_count} | {sub_ok} | "
            f"{r.get('response_time_ms', 0) or 0:.0f} | {r.get('output_tokens_est', 0)} |"
        )
    lines += [
        "",
        "## 4. 结论",
        "",
        f"- 总请求数 {total_requests}，符合 ≤30 的限流目标。",
        f"- 合并了 {total_sub} 个测试用例为 {total_requests} 次 API 调用。",
...
def main() -> None:
    ...
    if not args.skip_health_check:
        ...
        print(f"跳过健康检查（--skip-health-check），直接开始测试")
    ...
    all_batches.extend(generate_search_batches(batch_size=5, total=15))

    print(f"共 {len(all_batches)} 个批次（总请求数），合并 120 个测试用例")
