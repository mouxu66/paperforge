...
import type { CitationRelations, Note, NoteCreate, NoteUpdate } from "./types";
 ...
 */
export async function fetchCitationRelations(paperId: string): Promise<CitationRelations> {
  const { data } = await http.get<CitationRelations>(`/papers/${paperId}/citations`);
  return data;
}
