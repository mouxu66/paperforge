...
const SSE_BASE = import.meta.env.VITE_API_BASE || import.meta.env.VITE_ASK_API_BASE || '/api'
...
export async function fetchChapterTree(
  projectId: number,
): Promise<ChapterTreeNode[]> {
  const { data } = await http.get<ChapterTreeNode[]>(
    `/writing/projects/${projectId}/chapters`,
  )
  return data
}
...
export async function createChapter(
  ...
  const { data } = await http.post<Chapter>(
    `/writing/projects/${projectId}/chapters`,
    payload,
  )
  return data
...
export async function updateChapter(
  ...
  const { data } = await http.put<Chapter>(
    `/writing/chapters/${chapterId}`,
    payload,
  )
  return data
}
...
export async function moveChapter(
  ...
  const { data } = await http.put<Chapter>(
    `/writing/chapters/${chapterId}/move`,
    payload,
  )
  return data
}
...
export async function startExportTask(
  projectId: number,
): Promise<ExportTaskCreate> {
  const { data } = await http.post<ExportTaskCreate>(
    `/writing/projects/${projectId}/export`,
  )
  return data
}
...
export async function getExportProgress(
  taskId: string,
): Promise<ExportTaskProgress> {
  const { data } = await http.get<ExportTaskProgress>(
    `/writing/export/${taskId}/progress`,
  )
  return data
}
...
export async function fetchWordCount(
  ...
  const { data } = await http.get<WordCountResult>(
    `/writing/projects/${projectId}/word-count`,
  )
  return data
}
...
export async function fetchChapterVersions(
  ...
  const { data } = await http.get<ChapterVersion[]>(
    `/writing/chapters/${chapterId}/versions`,
  )
  return data
}
...
export async function fetchContinuations(
  chapterId: number,
