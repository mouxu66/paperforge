...
/** 导出为 Markdown（.md） */
export function exportMarkdown(content: string, refs: AskReference[], filename: string): void {
  let md = content;
  if (refs.length) {
    md += "\n\n## 参考文献\n\n";
...
export function exportWord(html: string, refs: AskReference[], filename: string): void {
...
/** 导出为 LaTeX（.tex） */
export function exportLatex(content: string, refs: AskReference[], filename: string): void {
  ...
  const latex =
    ...
    `\\maketitle\n\n` +
    `${latexBody}\n\n` +
    (refs.length ? `\\begin{thebibliography}{99}\n${bibItems}\n\\end{thebibliography}\n\n` : "") +
    `\\end{document}\n`;
...
export function exportPdf(html: string, refs: AskReference[], filename: string): void {
  // 🛡️ 全量审查 1-#11：原版 printWindow.document.write() 直接拼接 html + refsHtml。
  // 两者均走 markdownToHtml/escapeHtml 提前转义。**不要再次 escapeHtml(html)**——
  // 会被二次转义为 »&lt;strong&gt;Bold&lt;/strong&gt;« 在打印页直接显示。
  // null 检查是重点：浏览器拦截弹出窗口时 printWindow 为 null，要报错。
  const refsHtml = refsToHtml(refs);
  const printWindow = window.open("", "_blank");
  if (!printWindow) {
    console.error("[exportPdf] 浏览器拦截了弹出窗口 —— 请检查浏览器设置");
    return;
  }
