import { ArrowLeft, Download, Pencil, FileCode, FileType, FileText, Maximize, Minimize, Crosshair, MessageCircle, Save, Menu, BookOpen, HelpCircle } from "lucide-react";
...
import CslExportPanel from "@/components/writing/CslExportPanel";
import WritingGuide from "@/components/writing/WritingGuide";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import { renderTextWithCitations } from "@/components/writing/ReferencePreview.utils";
...
function WritingGuidePanel() {
  ...
  const GUIDE_GROUPS: GuideGroup[] = [
    ...
    },
    {
      ...
      color: "default",
      items: [{ name: t("editor.guideStartWriting"), desc: t("editor.guideStartWritingDesc") }],
    },
  ];
        ...
        items={[
          {
            ...
            label: (
              <Space size={8}>
                <span style={{ fontSize: 14, fontWeight: 600, color: "var(--pf-text-primary)" }}>
...
export default function WritingEditor() {
  ...
  const [exportChapterInfo, setExportChapterInfo] = useState<{
    current: number;
    total: number;
  } | null>(null);
  // A3: 导出预览（null 表示未在预览）
  ...
  // WP-3.1: CSL export panel
  const [cslModalOpen, setCslModalOpen] = useState(false);
  const [cslItems, setCslItems] = useState<CslItem[]>([]);
  ...
  /** 在大纲树中按 id 查找节点 */
  const findNode = (nodes: ChapterTreeNode[], id: number): ChapterTreeNode | null => {
  ...
  const handleMove = async (chapterId: number, parentId: number | null, index: number) => {
    try {
  ...
  const handleGenerate = async (chapterId: number) => {
    ...
    try {
      ...
      await loadTree();
      setSelected((cur) =>
        cur && cur.id === chapterId ? { ...cur, content: result.content } : cur,
      );
      // 生成后也刷新字数
      void loadWordCount();
    } catch {
  ...
  const handleOpenCslModal = async () => {
    setCslModalOpen(true);
    setCslLoading(true);
    try {
      const items = await exportProjectCsl(pid);
      setCslItems(items);
    } catch {
      // ignored
    } finally {
  ...
  const handleSetTarget = async () => {
    try {
      ...
      message.success(
        targetValue > 0 ? t("editor.targetSet", { count: targetValue }) : t("editor.targetCleared"),
      );
    } catch {
      // 错误已由 http 拦截器提示
    }
    ...
    try {
      ...
      for (let i = 0; i < 150; i++) {
        ...
        if (p.status === "done") {
          ...
          // 过滤掉被标记为隐藏的引用
          const refs = (p.references as ExportReference[]).filter((r) => !hiddenRefIds.has(r.id));
          // A3: 不直接下载，先弹出预览
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: isMobile ? "0 8px" : 0 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <Space size={isMobile ? 6 : 12} align="center" wrap>
          <Button type="text" icon={<ArrowLeft />} onClick={() => navigate("/write")} />
          <Pencil style={{ color: "var(--pf-primary)" }} />
          <Typography.Title
            level={4}
            className="pf-serif"
          ...
          <Tag color="blue">
            {project.chapterCount} {t("write.chapterUnit")}
          </Tag>
          {/* 移动端/平板：显示大纲按钮 */}
          {!isDesktop && (
            <Button
          ...
          >
            {!isMobile &&
              (targetWords > 0
                ? t("editor.targetWords", { count: targetWords.toLocaleString() })
                : t("editor.setTargetLabel"))}
          </Button>
          <Button
            size="small"
            type="text"
          ...
          >
            {!isMobile
              ? `${t("editor.notes")}${notes.length > 0 ? ` (${notes.length})` : ""}`
              : notes.length > 0
                ? ` ${notes.length}`
                : ""}
          </Button>
        ...
        {targetWords > 0 && (
          <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 8 }}>
            <Progress
              ...
              strokeColor={targetReached ? "var(--pf-success)" : "var(--pf-primary)"}
            ...
            <span
              style={{
                fontSize: 12,
                color: targetReached ? "var(--pf-success)" : "var(--pf-text-placeholder)",
              }}
            >
              {targetReached
                ? "🎉 " + t("editor.targetReached")
      ...
      >
        ...
        <Card
          ...
          title={
            <Space>
              <Download style={{ color: "var(--pf-primary)" }} />
        ...
        >
          {/* 导出进度条 */}
          {exportProgress !== null && (
            <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--pf-border)" }}>
              <Progress
                ...
                size="small"
              />
              <div style={{ marginTop: 4, fontSize: 12, color: "var(--pf-text-muted)" }}>
                {exportStatus === "error"
      ...
      {fullscreen && (
        <div className="pf-fullscreen-overlay">
          <div className="pf-fullscreen-toolbar">
            <span style={{ fontSize: 13, color: "var(--pf-text-placeholder)" }}>
              {t("editor.fullscreenHint", {
                title: selected?.title ?? "",
                count: totalWords.toLocaleString(),
              })}
            </span>
            <Button
              size="small"
      ...
      {/* 设定目标弹窗 */}
      {/* WP-3.1: CSL export modal */}
      <Modal
        title={
          <Space>
            <FileText />
            <span>{t("editor.cslExportTitle")}</span>
          </Space>
        }
        open={cslModalOpen}
        onCancel={() => setCslModalOpen(false)}
        width="80%"
        style={{ top: 20 }}
        footer={null}
      >
      ...
      >
        <div style={{ marginTop: 16 }}>
          <Typography.Text style={{ fontSize: 13, color: "var(--pf-text-muted)" }}>
      ...
      <Drawer
        title={
          <Space>
            <MessageCircle style={{ color: "#d97706" }} />
      ...
      >
        {notes.length === 0 ? (
          <Empty description={t("editor.notesEmpty")} style={{ marginTop: 60 }}>
            <Typography.Text type="secondary" style={{ fontSize: 12 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {notes.map((n) => (
              ...
              >
                ...
                <div style={{ marginTop: 8, fontSize: 11, color: "var(--pf-text-placeholder)" }}>
      ...
      <Modal
        title={
          <Space>
            <FileCode />
            <span>
              {t("editor.exportPreview")} {exportPreview?.format.toUpperCase()}
            </span>
          </Space>
        }
                ...
                >
                  {exportPreview.refs.map((r) => (
                    <li key={r.id} style={{ marginBottom: 4 }}>
                      <span style={{ color: "var(--pf-primary)", fontFamily: "monospace" }}>
                        [{r.id}]
                      </span>{" "}
                      {r.authors?.join(", ") || "Unknown"} ({r.year || "n.d."}).{" "}
