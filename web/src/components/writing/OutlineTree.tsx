...
export default function OutlineTree({
  ...
  const handleCreate = async () => {
    ...
    try {
      const parentId =
        addParent === 'root' || addParent === null ? null : addParent.id
  ...
  const handleDrop: TreeProps['onDrop'] = async (info) => {
    ...
    if (!info.dropToGap) {
      ...
      const siblings =
        dropNode.parentId === null
          ? tree
          : (nodeMap.get(dropNode.parentId)?.children ?? [])
  ...
  const titleRender = (node: DataNode) => {
    ...
    return (
      ...
      >
        <Space size={4}>
          {hasChildren ? (
            <FolderOpenOutlined style={{ color: '#1e40af', fontSize: 13 }} />
          ) : (
          ...
          <span
            ...
            style={{
              fontSize: 13,
              color: selectedId === id ? '#1e40af' : '#1a1a2e',
          ...
          {wordCountMap && (
            <WordCountBadge count={wordCountMap.get(id) ?? 0} />
          )}
        </Space>
