...
export default function ModelSelector() {
  ...
  const { current, available, enabled, switching, load, switchModel } =
    useModelStore()
  ...
  const options = available.map((m) => ({
    ...
    label: (
      <span style={{ fontWeight: m.value === current ? 600 : 400 }}>
        {m.label}
        {m.value === current && <span style={{ color: '#1e40af' }}>{t('models.current')}</span>}
      </span>
    ),
  }))
  ...
  return (
    <Tooltip title={t('models.switchTooltip')}>
      <Select
        ...
        size="middle"
        className={cls}
        suffixIcon={<ThunderboltOutlined style={{ color: '#1e40af' }} />}
        style={{ minWidth: 180 }}
