    ...
    >
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--pf-text-placeholder)", lineHeight: 1.6 }}>
            {refItem.authors && refItem.authors.length > 0 && (
              <>
                {formatAuthors(refItem.authors, 3)}
                {refItem.year && (
                  <span style={{ margin: "0 8px", color: "var(--pf-scrollbar)" }}>·</span>
                )}
              </>
            )}
        ...
        <ArrowRight style={{ color: "var(--pf-scrollbar)", fontSize: 13, marginTop: 8 }} />
      </div>
    </Card>
  );
}
